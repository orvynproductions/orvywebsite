...
import { facilityCarouselConfig } from '@/lib/config';

export default function FacilityCarousel() {
  const slides = facilityCarouselConfig.slides;
    ...
    >
      ...
      <div className="absolute inset-0 opacity-[0.03]">
        <div className="absolute inset-0" style={{
          backgroundImage: `linear-gradient(45deg, #c1a768 25%, transparent 25%), linear-gradient(-45deg, #c1a768 25%, transparent 25%)`,
      ...
      <div className="container-custom relative">
        ...
        <div className="fade-up text-center mb-12">
          <span className="font-script text-3xl text-gold-500 block mb-2">{facilityCarouselConfig.scriptText}</span>
          <span className="text-gold-500 text-xs uppercase tracking-[0.2em] mb-4 block">
            {facilityCarouselConfig.subtitle}
          </span>
          <h2 className="font-serif text-4xl md:text-5xl text-white">
            {facilityCarouselConfig.mainTitle}
          </h2>
        </div>
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-0 items-stretch">
              ...
              {slides.map((slide, index) => (
                ...
                >
                  <Image
                    ...
                    fill
                    className={`object-cover ${index === currentSlide ? 'kenburns' : ''}`}
                  />
            ...
            <div className="lg:bg-white/5 lg:border-y lg:border-r lg:border-white/10 lg:rounded-r-lg p-8 lg:p-12 flex flex-col justify-center relative overflow-hidden">
              {slides.map((slide, index) => (
                ...
                >
                  {/* Location Tag */}
                  {facilityCarouselConfig.locationTag && (
                    <div className="flex items-center gap-2 text-gold-500 text-sm mb-4">
                      <MapPin className="w-4 h-4" />
                      <span>{facilityCarouselConfig.locationTag}</span>
                    </div>
                  )}
